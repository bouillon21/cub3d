...
/*   Updated: 2021/02/18 20:58:23 by cshelli          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (mas[0] && !cub->pars.sHeight && !ft_strncmp(mas[0], "R", 2))
	{
		cub->pars.sWidth = ft_atoi(mas[1]);
		cub->pars.sHeight = ft_atoi(mas[2]);
	}
	else if (mas[0] && !cub->pars.NO && !ft_strncmp(mas[0], "NO", 3))
		cub->pars.NO = ft_strdup(mas[1]);
	else if (mas[0] && !cub->pars.SO && !ft_strncmp(mas[0], "SO", 3))
		cub->pars.SO = ft_strdup(mas[1]);
	else if (mas[0] && !cub->pars.WE && !ft_strncmp(mas[0], "WE", 3))
		cub->pars.WE = ft_strdup(mas[1]);
	else if (mas[0] && !cub->pars.EA && !ft_strncmp(mas[0], "EA", 3))
		cub->pars.EA = ft_strdup(mas[1]);
	else if (mas[0] && !cub->pars.S && !ft_strncmp(mas[0], "S", 2))
